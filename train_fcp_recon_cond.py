...
from alg.ppo import ppo_cond as ppo
...
def torch_callback(obs1,hx1,cx1,obs2,hx2,cx2,partner_ids,recon_model,agent,partner_base_model,partner_params,partner_buffers):
    ...
    with torch.inference_mode(),torch.no_grad():
        obs1_ = j2t(obs1).float().reshape(obs1.shape[0],1,-1).to(device)
        ...
        _,epismetic,aleatoric = recon_model(obs1_)
        uncertainty = (epismetic + aleatoric).squeeze(-1).sum(dim=-1)
        logit, value,(next_h1,next_c1) = agent(j2t(obs1).permute(0,3,1,2).float().to(device),uncertainty, hx1_, cx1_)
    ...
    return t2j(uncertainty),t2j(actions),t2j(log_prob),t2j(value.squeeze()), t2j(next_h1), t2j(next_c1), t2j(next_h2), t2j(next_c2)
...
def run_episode_p(env, key, episode_length, callback_fn,batch_size):
    ...
    def _step_fn(carry, _):
        ...
        def step_if_not_done(current_state,current_obs1,current_obs2):
            ...
            result_shape_dtype = (
                jax.ShapeDtypeStruct((batch_size,1), jnp.float32),      # reconobs
            ...
            uncertainty,policy_action, log_prob,value,hx1_,cx1_,hx2_,cx2_= io_callback(
                callback_fn,
                result_shape_dtype,
                obs1,hx1,cx1,
                obs2,hx2,cx2,
        ...
        next_obs1,next_obs2,next_state, reward, shaped_reward, next_done, chosen_actions,log_prob,value, hx1,cx1,hx2,cx2,partner_ids,uncertainty = jax.lax.cond(
            done.all(),
            skip_step_if_done,
            step_if_not_done,
        ...
        outputs = (obs1,reward, chosen_actions, shaped_reward, log_prob, value,uncertainty)
    ...
    obs1,rewards, actions, shaped_rewards,log_probs,values,uncertainty = outputs
    return obs1,rewards, actions, shaped_rewards,log_probs, values,uncertainty
...
def collect_episode(env,callback_fn,batch_size,seed,episode_length=400):
    with torch.no_grad():
        ...
        obss1,rewards, actions, shaped_rewards,log_probs,values,uncertainty = run_episode_p(env, key, episode_length,callback_fn,batch_size)
    return j2t(obss1[:-1]),j2t(actions[:-1]),j2t(rewards[:-1]),j2t(shaped_rewards[:-1]),j2t(log_probs[:-1]),j2t(values),j2t(uncertainty[:-1])


def process_episodes(obss,acts,uncertainty,model,old_log_probs,adv,device):
    ...
    logit_actions, means_values,(_,_) = model(obss,uncertainty, hx, cx)
...
if __name__ == "__main__":    
    ...
    device = "cuda:0"
    partner_pool = []
    model,_,_,_,_,_,_ = build_model(recon_config,device,dtype=torch.float16,device_type='cuda',task='classification',is_kv = True)
    ...
    for iter in tqdm(range(start_iter,config['params']['num_iterations'])):
        ...
        obss,acts,rwds,shaped1,log_probs,values,uncertainty = collect_episode(env,callback_fn,config['params']['batch_size'],
                                                                 base_seed + iter*config['params']['max_episode_length'],config['params']['max_episode_length'])
        obss = obss.float().permute(1,0,4,2,3).to(device)
        ...
        uncertainty = uncertainty.float().permute(1,0,2).to(device)
        with torch.no_grad():
        ...
        agent.train()
        for i in range(10):
            loss_actions, means_values, entropy = process_episodes(obss, acts, uncertainty, agent, log_probs, adv1, device)
