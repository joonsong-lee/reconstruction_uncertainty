...
def torch_callback(obs1,hx1,cx1,obs2,hx2,cx2,partner_ids,recon_model,agent,partner_base_model,partner_params,partner_buffers,added_dim):
    ...
    with torch.inference_mode(),torch.no_grad():
        ...
        uncertainty = (epismetic + aleatoric).squeeze(-1).reshape(obs1_.shape[0],40,5,11)
        if added_dim ==1:
            uncertainty = uncertainty.mean(dim=1,keepdim=True)
        recon_obs = recon_obs.reshape(obs1_.shape[0],40,5,11)
        obs1_ = torch.cat([recon_obs, uncertainty], dim=1)
...
def run_episode_p(env, key, episode_length, callback_fn,batch_size):
    ...
    def _step_fn(carry, _):
        ...
        def step_if_not_done(current_state,current_obs1,current_obs2):
            ...
            result_shape_dtype = (
                jax.ShapeDtypeStruct((batch_size,41,5,11), jnp.float32),      # reconobs
        ...
        outputs = (recon_obs,reward, chosen_actions, shaped_reward, log_prob, value)
