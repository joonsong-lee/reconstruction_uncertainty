...
def main(args):
    ...
    for iter in tqdm(range(start_iter,config['params']['num_iterations'])):
        ...
        if (iter+1)% config['log_interval'] == 0:
            wandb.log({
                'fcp_recon_nolstm_return': mean_rewards.item(),
                'fcp_recon_nolstm_loss_actions': losses.intermediate_losses['loss_actions'],
                'fcp_recon_nolstm_loss_values': losses.intermediate_losses['loss_values'],
                'fcp_recon_nolstm_loss_entropy': losses.intermediate_losses['loss_entropy'],
                'fcp_recon_nolstm_total_loss': total_loss.item(),
                'fcp_recon_nolstm_reg': reg.item(),
                'fcp_recon_nolstm_mse': mse.item(),
            },step=iter)
