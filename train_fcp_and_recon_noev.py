...
def torch_callback(obs1,hx1,cx1,obs2,hx2,cx2,partner_ids,recon_model,agent,partner_base_model,partner_params,partner_buffers,added_dim,ctx):
    ...
    with torch.inference_mode(),torch.no_grad():
        ...
        recon_obs = recon_obs.reshape(obs1_.shape[0],5,11,40)
...
def run_episode_p(env, key, episode_length, callback_fn,batch_size):
    ...
    def _step_fn(carry, _):
        ...
        def step_if_not_done(current_state,current_obs1,current_obs2):
            ...
            result_shape_dtype = (
                jax.ShapeDtypeStruct((batch_size,40,5,11), jnp.float32),      # reconobs
...
def main(args):
    ...
    fcp_keys = ['best_model.pt','model_iter_199.pt','model_iter_1199.pt']
    ...
    env = make("overcooked_v2", layout=l, agent_view_size = 2,random_agent_positions=True,random_reset = False,
               max_steps=config['params']['max_episode_length'],sample_recipe_on_delivery=True,indicate_successful_delivery=False,negative_rewards=False)
