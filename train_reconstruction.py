...
class rc_trainer:
    def __init__(self,model,optimizer,scaler,ctx,train_sampler,train_loader,val_loader,config,local_rank,ddp_world_size,best_val_loss,device):
        ...
        self.save = 0
        self.denoiser = np.clip(np.array([[[[ 1,  1,  1,  1,  1,  1,  1,  3,  3,  3,  1,  1,  1,  1,  1,
    ...
    """
            ...
            self.env = make("overcooked_v2", layout=l, agent_view_size = None,random_agent_positions=True,random_reset = False,max_steps=400,sample_recipe_on_delivery=True)

    def train_step(self, iternum,data):
        X,Y = data
        with self.ctx:
            (mu,v,alpha,beta),(total_loss,nll_loss,reg,mse) = self.model(X.to(self.device), Y.to(self.device))
        ...
        return total_loss,nll_loss,reg,mse
    ...
    def log_and_save(self, metrics_avg, epoch, iter_num, total_iter):
        wandb.log({
            ...
            f"val_nll_loss": metrics_avg['val_loss_nll'].item(),
    ...
    def evaluate_and_save(self, epoch, iter_num, total_iter,partial=False):
        ...
        with torch.no_grad(), self.ctx:
            for j,(X, Y) in enumerate(self.val_loader):
                (mu,v,alpha,beta),(total_loss,nll_loss,reg,mse_loss) = self.model(X.to(self.device), Y.to(self.device))
                epistemic_uncertainty, aleatoric_uncertainty = self.model.module.uncertainty(mu, v, alpha, beta)
                metrics_avg['val_loss_total'] += total_loss
                metrics_avg['val_mse'] += mse_loss
                metrics_avg['val_loss_nll'] += nll_loss
        ...
        if self.master_process:
            ...
            if ((total_iter//self.config['eval_every']) % self.config['save_interval'] == 0):
                ...
                y_obs = Y[rand_idx].cpu().numpy().reshape(400,5,11,40) * self.denoiser
                mu_sample = mu[rand_idx].cpu().numpy().reshape(400,5,11,40) * self.denoiser
                seq_to_seq_viz(self.env,y_obs,mu_sample,filename=f'{self.out_dir}/recon_iter{total_iter}')
...
def main(args):
    ...
    train_dataset = rc_dataset('regression','train',**config['params']['dataset'])
    val_dataset = rc_dataset('regression','val',**config['params']['dataset'])
    ...
    model,optimizer,scaler,total_iter,epoch,iter_num,best_val_loss = build_model(config,device,dtype,device_type,task='regression',is_kv = False)
