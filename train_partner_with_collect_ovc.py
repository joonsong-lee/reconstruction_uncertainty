...
def run_episode_p(env, key, episode_length, callback_fn,save_path,batch_size):
    ...
    def _step_fn(carry, _):
        ...
        outputs = (obs1,obs2,reward, chosen_actions, shaped_reward, shaped_reward2, log_prob, value, log_prob2,g1)
    ...
    obs1,obs2,rewards, actions, shaped_rewards,shaped_rewards2,log_probs,values,log_probs2,g1 = outputs
    return obs1,obs2,rewards, actions, shaped_rewards, shaped_rewards2, log_probs, values, log_probs2, g1
...
def collect_episode(callback_fn,env,batch_size,seed,save_path,episode_length=400):
    with torch.no_grad():
        ...
        obss1,obss2, rewards, actions, shaped_rewards,shaped_rewards2,log_probs,values,log_probs2,g1 = run_episode_p(env, key, episode_length,callback_fn,save_path,batch_size)
    actions1 = actions[:-1,:,0]
    actions2 = actions[:-1,:,1]
    if save_path is not None:
        ...
        for i in range(batch_size):
            ...
            g1 = np.memmap(os.path.join(save_path,'global', f'{seed}.npy'), mode='w+', dtype=np.uint8, shape=(episode_length, 6,8,41))
            o1[:] = np_obss1.squeeze()[:,i]
            g1[:] = globals.squeeze()[:,i]
            o1.flush()
            g1.flush()
    return j2t(obss1[:-1]),j2t(obss2[:-1]), j2t(actions1),j2t(actions2), j2t(rewards[:-1]), j2t(shaped_rewards[:-1]),j2t(shaped_rewards2[:-1]), j2t(log_probs[:-1]), j2t(values), j2t(log_probs2[:-1])


def process_episodes(obss,obss2,acts,acts2,model,old_log_probs,old_log_probs2,adv,adv2,device):
    ...
    loss1_2 = ratio2 * adv2
    loss2_2 = torch.clamp(ratio2, 1 - 0.1, 1 + 0.1) * adv2
...
def main(args):
    ...
    config['params']['base_seed'] = args.agent_num * 100000
    base_seed = config['params']['base_seed']
    print(f'Base seed: {base_seed}')
    config['path']['out_dir'] = config['path']['out_dir'] + str(args.agent_num)
    ...
    for iter in tqdm(range(start_iter,config['params']['num_iterations'])):
        agent.reset_hidden(batch_size=config['params']['batch_size'])
        obss,obss2,acts,acts2,rwds,shaped1,shaped2,log_probs,values,log_probs2 = collect_episode(callback_fn,env,config['params']['batch_size'],
                                                                                                 base_seed+iter*config['params']['batch_size'],None if iter<1000 else save_path,
                                                                                                 config['params']['max_episode_length'])
        
        obss = obss.float().permute(1,0,4,2,3).to(device)
        obss2 = obss2.float().permute(1,0,4,2,3).to(device)
        acts = acts.squeeze().long().permute(1,0).to(device)
        acts2 = acts2.squeeze().long().permute(1,0).to(device)
        rwds = rwds.squeeze().float().permute(1,0).to(device)
        shaped1 = shaped1.squeeze().float().permute(1,0).to(device)
        shaped2 = shaped2.squeeze().float().permute(1,0).to(device)
        log_probs = log_probs.squeeze().float().permute(1,0).to(device)
        values = values.squeeze().float().permute(1,0).to(device)
        log_probs2 = log_probs2.squeeze().float().permute(1,0).to(device)
        with torch.no_grad():
            lambda_return,_= compute_lambda_returns(
                rwds= rwds,
                values= values,
            ...
            _,adv1 = compute_lambda_returns(
                rwds= rwds+shaped1,
                values= values,
                gamma=config['params']['loss']['gamma'],
                lambda_=config['params']['loss']['lambda_'],
            )
            _,adv2 = compute_lambda_returns(
                rwds= rwds+shaped2,
                values= values,
                gamma=config['params']['loss']['gamma'],
                lambda_=config['params']['loss']['lambda_'],
        ...
        agent.train()
        for i in range(10):
            loss_actions, means_values, entropy = process_episodes(obss,obss2, acts,acts2, agent, log_probs,log_probs2, adv1,adv2, device)
