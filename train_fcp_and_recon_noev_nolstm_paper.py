...
def torch_callback(obs1,obs2,hx2,cx2,partner_ids,recon_model,agent,partner_base_model,partner_params,partner_buffers,added_dim,ctx):
    ...
    with torch.inference_mode(),torch.no_grad():
        ...
        with ctx:
            recon_obs,_ = recon_model(obs1_)
        recon_obs = recon_obs.reshape(obs1_.shape[0],5,8,41)
        obs1_ = recon_obs.permute(0,3,1,2).float()
        logit, value= agent(obs1_)
    ...
    return t2j(obs1_),t2j(actions),t2j(log_prob),t2j(value.squeeze()), t2j(next_h2), t2j(next_c2)
...
def run_episode_p(env, key, episode_length, callback_fn,batch_size):
    ...
    def _step_fn(carry, _):
        key, state, done, obs1, obs2, hx2, cx2,partner_ids = carry
        ...
        def step_if_not_done(current_state,current_obs1,current_obs2):
            ...
            result_shape_dtype = (
                jax.ShapeDtypeStruct((batch_size,41,5,8), jnp.float32),      # reconobs
            ...
            recon_obs,policy_action, log_prob,value,hx2_,cx2_= io_callback(
                callback_fn,
                result_shape_dtype,
                obs1,
                obs2,hx2,cx2,
        ...
        def skip_step_if_done(current_state,current_obs1,current_obs2):
            ...
            # Wrapper so we only pass JAX arrays (render_state, hx, cx); vae & ac captured (static)
            recob_obs,policy_action, log_prob,value,hx2_,cx2_= io_callback(
                callback_fn,
                result_shape_dtype,
                obs1,
                obs2,hx2,cx2,
        ...
        next_obs1,next_obs2,next_state, reward, shaped_reward, next_done, chosen_actions,log_prob,value, hx2,cx2,partner_ids,recon_obs = jax.lax.cond(
            done.all(),
            skip_step_if_done,
            step_if_not_done,
        ...
        g1 = g[:,0]
        new_carry = (key, next_state, next_done, next_obs1,next_obs2,hx2,cx2,partner_ids)
    ...
    initial_carry = (key, initial_state, initial_dones, initial_obs1,initial_obs2,  initial_hx2, initial_cx2, partner_ids)
...
def collect_episode(env,callback_fn,batch_size,seed,episode_length=400):
    ...
    return j2t(obss1),j2t(actions[:-1]),j2t(rewards[:-1]),j2t(shaped_rewards[:-1]),j2t(log_probs[:-1]),j2t(values),j2t(obs1[:,:16]),j2t(g[:,:16])


def process_episodes(obss,acts,model,old_log_probs,adv,device):
    logit_actions, means_values = model(obss)
...
def main(args):
    ...
    recon_device = "cuda:0"
    partner_pool = []
    model,recon_optimizer,recon_scaler,_,_,_,_ = build_model(recon_config,recon_device,dtype=torch.float16,device_type='cuda',task='no_evidential_classification',is_kv = True)
    ...
    for iter in tqdm(range(start_iter,config['params']['num_iterations'])):
        ...
        if buffer.size>= config['params']['buffer_size']:
            ...
            total_loss = train_step((X,Y),model,recon_optimizer,recon_scaler,0.1,ctx)
        else:
        ...
        if  (iter) % config['params']['save_interval'] == 0:
            torch.save({
