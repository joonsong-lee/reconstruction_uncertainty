...
def torch_callback(obs1,hx1,cx1,obs2,hx2,cx2,partner_ids,recon_model,agent,partner_base_model,partner_params,partner_buffers,added_dim,ctx):
    ...
    with torch.inference_mode(),torch.no_grad():
        ...
        epismetic = epismetic.squeeze(-1).reshape(obs1_.shape[0],5,11,40).to(device)
        aleatoric = aleatoric.squeeze(-1).reshape(obs1_.shape[0],5,11,40).to(device)
        if added_dim ==1:
        ...
        recon_obs = recon_obs.reshape(obs1_.shape[0],5,11,40).to(device)
...
def run_episode_p(env, key, episode_length, callback_fn,batch_size):
    ...
    def _step_fn(carry, _):
        ...
        def step_if_not_done(current_state,current_obs1,current_obs2):
            ...
            result_shape_dtype = (
                jax.ShapeDtypeStruct((batch_size,42,5,11), jnp.float32),      # reconobs
